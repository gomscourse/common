...
func TestQueryPrettier(t *testing.T) {
	...
	t.Run(
		"without squirrel", func(t *testing.T) {
			t.Parallel()
			t.Run(
				"query with no params", func(t *testing.T) {
					t.Parallel()
			...
			t.Run(
				"query with spaces", func(t *testing.T) {
					t.Parallel()
			...
			t.Run(
				"select by one param", func(t *testing.T) {
					t.Parallel()
			...
			t.Run(
				"select with IN clause", func(t *testing.T) {
					t.Parallel()
					...
					require.Equal(
						t,
						fmt.Sprintf("SELECT id, author, chat_id FROM message WHERE id IN (%v,%v,%v)", id, 123, 321),
						pretty,
					)
				},
			)
			...
			t.Run(
				"insert", func(t *testing.T) {
					...
					require.Equal(
						t,
						fmt.Sprintf("INSERT INTO message (author, chat_id) VALUES (%v,%v)", safeName, chatID),
						pretty,
					)
				},
			)
			...
			t.Run(
				"update", func(t *testing.T) {
					...
					require.Equal(
						t,
						fmt.Sprintf(
							"UPDATE message SET author = %v, chat_id = %v WHERE author = %v",
							safeName,
							chatID,
							safeName,
						),
	...
	t.Run(
		"with squirrel", func(t *testing.T) {
			t.Parallel()
			t.Run(
				"query with no params", func(t *testing.T) {
					t.Parallel()

					builder := sq.Select("id, author, chat_id").
